...
import { MINIMUM_FARE } from '../config/constants';
...
export const ServiceSelectionSheet: React.FC<ServiceSelectionSheetProps> = ({
    ...
    const handleRequestClick = () => {
        ...
        if (service) {
            ...
            if (service.pricePerKm && routeDistanceKm !== null) {
                const calculatedFare = service.pricePerKm * routeDistanceKm;
                estimatedPrice = Math.round(Math.max(service.minFare || MINIMUM_FARE, calculatedFare));
            }
    ...
    const renderServiceList = () => {
        if (activeCategoryServices.length === 0) {
            return <p style={{ textAlign: 'center', color: '#777', padding: '1rem' }}>{t.noServicesAvailable}</p>;
        }

        return activeCategoryServices.map(service => {
            const ServiceImage = service.imageComponent;
            let priceDisplay: React.ReactNode;

            if (isCalculatingDistance) {
                priceDisplay = <div style={priceLoadingErrorStyle}>{t.calculatingPrice}</div>;
            } else if (distanceError) {
                priceDisplay = <div style={{ ...priceLoadingErrorStyle, color: 'red' }}>{distanceError}</div>;
            } else if (service.pricePerKm && routeDistanceKm !== null) {
                const calculatedFare = service.pricePerKm * routeDistanceKm;
                const estimatedPrice = Math.round(Math.max(service.minFare || MINIMUM_FARE, calculatedFare));
                priceDisplay = <div style={serviceItemPriceStyle}>{`${estimatedPrice.toLocaleString(currentLang === 'fa' || currentLang === 'ps' ? 'fa-IR' : 'en-US')} ${t.priceUnit}`}</div>;
            } else {
                priceDisplay = <div style={priceLoadingErrorStyle}>-</div>;
            }

            return (
                <div
                    key={service.id}
                    style={serviceItemStyle(service.id === selectedServiceId)}
                ...
                >
                    <div style={serviceItemImageStyle}><ServiceImage style={{ width: '90%', height: '90%' }} /></div>
                    <div style={serviceItemDetailsStyle}>
                        <div style={serviceItemNameStyle}>{t[service.nameKey] || t.defaultServiceName}</div>
                        <div style={serviceItemDescStyle}>{t[service.descKey] || t.defaultServiceDesc}</div>
                    </div>
                    {priceDisplay}
                </div>
            );
    ...
    return (
        <div style={sheetStyle} ref={sheetRef} role="dialog" aria-modal="true" aria-labelledby="service-sheet-title">
            <div style={tabsContainerStyle}>
                {serviceCategories.map(category => (
                    <button
                        key={category.id}
                        style={tabStyle(category.id === activeCategoryId)}
            ...
            <div style={optionsContainerStyle}>
                <div style={optionItemStyle} role="button" tabIndex={0} aria-disabled="true">
                    <FilterIcon style={optionIconStyle} /> {t.rideOptions}
                </div>
                <div style={optionItemStyle} role="button" tabIndex={0} aria-disabled="true">
                    <TagIcon style={optionIconStyle} /> {t.coupon}
                </div>
            </div>
            <div style={footerStyle}>
                <button
                    style={{ ...requestButtonStyle, ...(!selectedServiceId || isCalculatingDistance || distanceError ? requestButtonDisabledStyle : {}) }}
                    onClick={handleRequestClick}
                    disabled={!selectedServiceId || isCalculatingDistance || !!distanceError}
                >
                    {t.requestRideButtonText}
                </button>
                <button style={scheduleButtonStyle} aria-label={t.scheduledRideButtonAriaLabel} disabled={true}>
                    <ScheduledRideIcon style={{ color: '#555' }} />
                </button>
            </div>
        </div>
    );
