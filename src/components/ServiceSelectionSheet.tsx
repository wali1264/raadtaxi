...
export const ServiceSelectionSheet: React.FC<ServiceSelectionSheetProps> = ({
  ...
  const handleRequestClick = () => {
    ...
    if (service) {
      ...
      if (service.pricePerKm && routeDistanceKm !== null) {
        ...
        estimatedPrice = Math.max(MINIMUM_FARE, calculatedFare);
      } else if (service.price) {
        estimatedPrice = Math.max(MINIMUM_FARE, service.price);
      }
  ...
  return ( <div style={sheetStyle} ref={sheetRef} role="dialog" aria-modal="true" aria-labelledby="service-sheet-title"> <div style={tabsContainerStyle}> {serviceCategories.map(category => ( <button key={category.id} style={tabStyle(category.id === activeCategoryId)} onClick={() => { setActiveCategoryId(category.id); }} role="tab" aria-selected={category.id === activeCategoryId} > {t[category.nameKey] || category.id} </button> ))} </div> <div style={serviceListStyle}> {activeCategoryServices.length > 0 ? activeCategoryServices.map(service => { const ServiceImage = service.imageComponent; let priceDisplay: React.ReactNode; if (isCalculatingDistance) { priceDisplay = <div style={priceLoadingErrorStyle}>{t.calculatingPrice}</div>; } else if (distanceError) { priceDisplay = <div style={{...priceLoadingErrorStyle, color: 'red'}}>{distanceError}</div>; } else if (service.pricePerKm && routeDistanceKm !== null) { const calculatedFare = service.pricePerKm * routeDistanceKm; const estimatedPrice = Math.round(Math.max(MINIMUM_FARE, calculatedFare));
            priceDisplay = <div style={serviceItemPriceStyle}>{`${Math.round(estimatedPrice).toLocaleString(currentLang === 'fa' || currentLang === 'ps' ? 'fa-IR' : 'en-US')} ${t.priceUnit}`}</div>; } else if (service.price) { const fixedPrice = Math.max(MINIMUM_FARE, service.price);
