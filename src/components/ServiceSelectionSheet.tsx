...
export const ServiceSelectionSheet: React.FC<ServiceSelectionSheetProps> = ({
  ...
  const handleRequestClick = () => {
    ...
    if (service) {
      ...
      if (service.pricePerKm !== undefined && routeDistanceKm !== null) {
        const calculatedPrice = (service.price || 0) + (service.pricePerKm * routeDistanceKm);
        estimatedPrice = Math.max(calculatedPrice, service.minFare || 0);
      } else if (service.price !== undefined) {
        estimatedPrice = Math.max(service.price, service.minFare || 0);
      }
